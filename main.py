...
<style>
    ...
    /* ロゴヘッダーの固定(Sticky) - border-bottomを削除 */
    .header-sticky {{
...
EVENTS_DATA = {"2026-04-10": "🇺🇸 米CPI発表", "2026-04-30": "🇺🇸 FOMC金利発表", "2026-04-28": "🇯🇵 日銀発表"}
...
for col, now, cc, s_key, suffix, title in [(c1, n_ny, "US", "v_us", "us", L["us_m"]), (c2, n_jp, "JP", "v_jp", "jp", L["jp_m"])]:
    with col:
        ...
        for w in cal:
            ...
            for i, d in enumerate(w):
                if d == 0: h_table += '<td></td>'
                    ...
                    h_table += f'<td><span class="{ev_mark}" style="color:{d_c} !important;">{d_ui}</span></td>'
            h_table += '</tr>'
        ...
        with st.container(border=True):
            ...
            st.markdown('<div style="height:250px; overflow-y:auto;">' + "".join([f'<div class="item-row">{n}</div>' for n in AI_NEWS_LIST[cc]]) + '</div>', unsafe_allow_html=True)
