...
<style>
    ...
    /* ヘッダー下の余白を詰める調整 */
    .block-container {{ padding-top: 3.8rem !important; margin-top: -20px !important; }}
...
with c_left:
    ...
    for row in cal_u:
        ...
        for i, d in enumerate(row):
            if d == 0: h_tab_u += '<td></td>'
                ...
                d_val = f'<span class="today-marker">{d}</span>' if curr == now_ny.date() else str(d)
                h_tab_u += f'<td><span style="color:{d_c} !important; font-weight:800;">{d_val}</span></td>'
    ...
    with st.container(border=True):
        ...
        evs_u = {"2026-04-10": "🇺🇸 米CPI発表", "2026-04-30": "🇺🇸 FOMC発表", "2026-05-01": "🇺🇸 米雇用統計"}
        m_ev_u = [f'<div class="item-row"><b>{k[8:]}日</b>: {v}</div>' for k,v in sorted(evs_u.items()) if k.startswith(v_u.strftime("%Y-%m"))]
    ...
    with st.container(border=True):
        ...
        st.markdown('<div style="height:200px; overflow-y:auto;">' + "".join([f'<div class="item-row">{i+1}. 米国AI・テック最新ニュース {i+1}</div>' for i in range(10)]) + '</div>', unsafe_allow_html=True)
...
with c_right:
    ...
    for row in cal_j:
        ...
        for i, d in enumerate(row):
            if d == 0: h_tab_j += '<td></td>'
                ...
                d_val = f'<span class="today-marker">{d}</span>' if curr == now_jp.date() else str(d)
                h_tab_j += f'<td><span style="color:{d_c} !important; font-weight:800;">{d_val}</span></td>'
    ...
    with st.container(border=True):
        ...
        evs_j = {"2026-04-28": "🇯🇵 日銀発表", "2026-04-29": "🇯🇵 昭和の日", "2026-05-03": "🇯🇵 憲法記念日"}
        m_ev_j = [f'<div class="item-row"><b>{k[8:]}日</b>: {v}</div>' for k,v in sorted(evs_j.items()) if k.startswith(v_j.strftime("%Y-%m"))]
        st.markdown('<div style="height:120px; overflow-y:auto;">' + ("".join(m_ev_j) if m_ev_j else "予定なし") + '</div>', unsafe_allow_html=True)

    # AIニュース (日本)
    with st.container(border=True):
        st.markdown(f'<div class="box-header">{L["news_title"]}</div>', unsafe_allow_html=True)
        st.markdown('<div style="height:200px; overflow-y:auto;">' + "".join([f'<div class="item-row">{i+1}. 日本国内AI関連ニュース {i+1}</div>' for i in range(10)]) + '</div>', unsafe_allow_html=True)
