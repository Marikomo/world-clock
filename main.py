...
for col, now, cc, s_key, suffix, title in [(c1, n_ny, "US", "v_us", "us", L["us_m"]), (c2, n_jp, "JP", "v_jp", "jp", L["jp_m"])]:
    with col:
        ...
        st.markdown(f'<div style="font-weight:900; font-size:1.1rem; margin-bottom:15px; color:#000000;">{now.strftime("%Y/%m/%d %H:%M:%S")} {f"（{dst_info}）" if dst_info else ""}</div>', unsafe_allow_html=True)
        ...
        # ニュース・イベントデータ
        AI_NEWS = {"US": [f"{i+1}. 米国AI最新動向ニュース {i+1}" for i in range(10)], "JP": [f"{i+1}. 国内AI最新動向ニュース {i+1}" for i in range(10)]}
        
        with st.container(border=True):
            st.markdown(f'<div class="box-header">{view.month}月 注目の株式イベント</div>', unsafe_allow_html=True)
            m_ev = [f'<div class="item-row"><b>{k[8:]}日</b>: {v}</div>' for k,v in sorted({"2026-04-10": "🇺🇸 米CPI発表", "2026-04-30": "🇺🇸 FOMC発表"}.items()) if k.startswith(view.strftime("%Y-%m"))]
