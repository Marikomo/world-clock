...
for name, d in prices.items():
    ...
    s = "▲" if d['diff'] >= 0 else "▼"
    p_html += f'<div class="flex-item price-box"><div class="price-label">{name}</div><div class="price-val">{d["val"]:,.0f}</div><div class="{c}" style="font-size:0.55rem;">{s}{abs(d["diff"]):.0f}</div></div>'
p_html += '</div>'
