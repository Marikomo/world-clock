...
# --- 4. 究極の時刻取得ロジック (強制計算版) ---
# UTC（世界標準時）を全計算のベースにする
utc_now = datetime.now(pytz.utc)

# ニューヨーク (ET) 
# 現在(4月)はサマータイム確定のため、UTC-4時間を強制適用
n_ny = utc_now - timedelta(hours=4)

# 日本 (JST)
# UTC+9時間を強制適用
n_jp = utc_now + timedelta(hours=9)
...
for col, now, cc, s_key, suffix, title in [(c1, n_ny, "US", "v_us", "us", L["us_m"]), (c2, n_jp, "JP", "v_jp", "jp", L["jp_m"])]:
    with col:
        ...
        # 市場ステータス (now は timezone 情報を持たない単純 datetime として扱う)
        ...
        # 時計表示
        dst_label = L["dst_on"] if cc=="US" else ""
