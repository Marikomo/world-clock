...
<style>
    ...
    .header-row {{ display: flex; justify-content: space-between; align-items: center; padding: 10px 0; margin-bottom: 5px; }}
    .header-logo-title {{ font-family: 'Inter', sans-serif; font-size: 1.4rem; font-weight: 900; color: #111; letter-spacing: -0.04em; }}
    ...
    .status-main {{ display: flex; align-items: center; gap: 15px; }}
    .status-next {{ font-size: 0.95rem; color: #666; font-weight: 700; }}

    .cal-info-header {{ display: flex; align-items: baseline; gap: 8px; margin-bottom: 8px; flex-wrap: wrap; }}
    ...
    .cal-dst {{ font-size: 0.75rem; font-weight: 800; color: #fff; background: #333; padding: 2px 8px; border-radius: 3px; }}

    /* 価格ボードの一行強制 */
    .indicator-row [data-testid="column"] {{
    ...
    /* カレンダー操作ボタンの完全配置修正 */
    .button-container [data-testid="stHorizontalBlock"] {{
    ...
    .button-container [data-testid="column"]:nth-child(1) {{ text-align: left !important; }}
    .button-container [data-testid="column"]:nth-child(2) {{ text-align: center !important; }}
    .button-container [data-testid="column"]:nth-child(3) {{ text-align: right !important; }}
    
    .stButton > button {{
        border-radius: 0px !important; border: 1px solid #ccc !important;
        height: 38px; font-weight: 700; font-size: 0.75rem;
        width: 80px !important; /* ボタン幅を固定して左右端を合わせやすくする */
    }}
    ...
    @media (max-width: 600px) {{
        ...
        .stButton > button {{ width: 70px !important; }}
    }}
</style>
...
def get_market_status(now, m_type):
    ...
    return f'<div class="status-line" style="background-color: {bg};"><div class="status-main"><span>{st_text}</span><span class="status-next">{next_open_str}</span></div></div>'
