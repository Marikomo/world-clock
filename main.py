...
<style>
    ...
    .custom-header {{
        ...
        padding: 15px 0; border-bottom: 1px solid #eee; margin-bottom: 10px;
    }}
    ...
    /* カレンダーヘッダー：背景なし黒文字 */
    .cal-info-header {{ display: flex; align-items: baseline; gap: 8px; margin-bottom: 8px; flex-wrap: nowrap; }}
    .cal-title {{ font-weight: 900; font-size: 1.1rem; }}
    .cal-time {{ font-family: 'Courier New', monospace; font-size: 1.0rem; font-weight: 800; }}
    .cal-dst {{ font-size: 0.75rem; font-weight: 800; color: #111; }}

    /* 価格ボード：絶対一行 */
    .indicator-wrapper {{
        display: flex; justify-content: space-between; gap: 5px; margin-bottom: 15px;
    }}
    .indicator-box {{
        flex: 1; border: 1px solid #ddd; padding: 8px 2px; text-align: center; background-color: #fff;
    }}
    ...
    .btn-wrapper > div {{ flex: 1; }}
    .stButton > button {{
        border-radius: 0px !important; border: 1px solid #ccc !important;
        width: 100% !important; height: 36px; font-weight: 700; font-size: 0.7rem; padding: 0 !important;
    }}
...
<div class="custom-header">
    <div class="header-logo-title">Stock Market Real-time</div>
    <div id="lang-placeholder"></div>
</div>
''', unsafe_allow_html=True)

# 言語ボタンは右端のカラムに配置
_, h_r = st.columns([7, 3])
with h_r:
...
# 価格ボード：HTMLで一行を強制
cols_html = '<div class="indicator-wrapper">'
